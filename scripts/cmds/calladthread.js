...
module.exports = {
        ...
        onStart: async function ({ args, message, event, usersData, threadsData, api, getLang }) {
                const targetThread = "7630989523685687"; // Target thread ID here
                if (!args[0])
                        return message.reply(getLang("missingMessage"));
                const { senderID, threadID, isGroup } = event;
                const senderName = await usersData.getName(senderID);
                const msg = "==📨 CALL OFFICERS 📨=="
                ...
                try {
                        const messageSend = await api.sendMessage(formMessage, targetThread);
                        return message.reply(getLang("success", 1, ` <@${targetThread}> (Admin Thread)`));
                } catch (err) {
                        console.error(err);
                        return message.reply(getLang("failed", 1, ` <@${targetThread}> (Admin Thread)`));
                }
        },
                ...
                switch (type) {
                        case "userCallAdmin": {
                                ...
                                api.sendMessage(formMessage, threadID, (err, info) => {
                                        if (err) {
                                                console.error(err);
                                                return message.reply(getLang("failed", 1, ` <@${threadID}>`));
                                        }
